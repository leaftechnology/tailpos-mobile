...
class Sales extends React.PureComponent {
  ...
  render() {
    const {
      ...
      let tax = enableOverallTax
          ? receiptDefault.get_tax_total.toFixed(2)
          : receiptDefault.get_tax_total_based_on_each_item.toFixed(2);
      let roundOffValue = 0;
      let totalPayment = receiptDefault ? parseFloat(receiptDefault.netTotal.toFixed(2)) + parseFloat(tax, 10) : 0;
      if (roundOff){
