    ...
    >
      ...
      >
        <Grid>
          ...
          <Row>
            <Col>
              <Text>Total Net Sales with VAT</Text>
            </Col>
            <Col style={{ alignItems: "flex-end" }}>
              <Text>
                  {props.isCurrencyDisabled
                      ? formatNumber(parseFloat(props.report.totalNetSales, 10) + parseFloat(props.report.totalTaxes, 10))
                      : new MoneyCurrency(
                          props.currency ? props.currency : "PHP",
                      ).moneyFormat(
                          formatNumber(parseFloat(props.report.totalNetSales, 10) + parseFloat(props.report.totalTaxes, 10)),
          ...
          <Row>
            ...
            {JSON.parse(props.report.total_taxes).length > 0 ? (
                <FlatList
                    numColumns={1}
                    data={JSON.parse(props.report.total_taxes)}
                    ...
                    renderItem={({item}) =>
                        <Row>
                          ...
                          <Col style={{ alignItems: "flex-end" }}>
                            <Text>
                                {props.isCurrencyDisabled
                                    ? formatNumber(parseFloat(item.totalAmount, 10))
                                    : new MoneyCurrency(
                                        props.currency ? props.currency : "PHP",
                                    ).moneyFormat(
                                        formatNumber(parseFloat(item.totalAmount, 10)),
            ...
            {props.loyaltyProgram ? (
                <Row>
                  ...
                  <Col style={{ alignItems: "flex-end" }}>
                    <Text>
                        {props.isCurrencyDisabled
                            ? formatNumber(parseFloat(props.report.loyalty, 10))
                            : new MoneyCurrency(
                                props.currency ? props.currency : "PHP",
                            ).moneyFormat(
                                formatNumber(parseFloat(props.report.loyalty, 10)),
